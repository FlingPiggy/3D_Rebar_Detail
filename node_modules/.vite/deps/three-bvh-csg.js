...
function edgesToIndices(edges, outputVertices, outputIndices, epsilonScale) {
  ...
  for (let i = 0, l = edges.length; i < l; i++) {
    ...
    for (let i1 = i + 1; i1 < l; i1++) {
      ...
      const dist = edge0.distanceSqToLine3(edge1, _vec5, _vec22);
  ...
  for (let i = 0, l = edges.length; i < l; i++) {
    ...
    for (let v = 0, lv = outputVertices.length; v < lv; v++) {
      const vec2 = outputVertices[v];
      const param = edge.closestPointToPointParameter(vec2, true);
      edge.at(param, _vec5);
      if (vec2.distanceToSquared(_vec5) < RELATIVE_EPSILON * epsilonScale) {
...
var CDTTriangleSplitter = class {
  ...
  _to2D(point, target) {
    ...
    _vec5.subVectors(point, projOrigin);
    return target.set(_vec5.dot(projU), _vec5.dot(projV), 0);
  }
  _from2D(u, v, target) {
    const { projOrigin, projU, projV } = this;
  ...
  triangulate() {
    ...
    let epsilonScale = 0;
    for (let i = 0; i < 3; i++) {
      const v = this._to2D(baseTri.points[i], _vec5);
...
var LegacyTriangleSplitter = class {
  ...
  initialize(tri) {
    ...
    if (Array.isArray(tri)) {
      for (let i = 0, l = tri.length; i < l; i++) {
        ...
        if (i === 0) {
          t.getNormal(normal);
        } else if (Math.abs(1 - t.getNormal(_vec6).dot(normal)) > EPSILON2) {
  ...
  splitByTriangle(triangle4, isCoplanar) {
    ...
    if (isCoplanar) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        _vec6.subVectors(v1, v0).normalize();
        _planeNormal.crossVectors(_triangleNormal, _vec6);
  ...
  splitByPlane(plane, clippingTriangle) {
    ...
    for (let i = 0, l = triangles.length; i < l; i++) {
      ...
      for (let t = 0; t < 3; t++) {
        ...
        let didIntersect = !!plane.intersectLine(_edge, _vec6);
        if (!didIntersect && Math.abs(endDist) < COPLANAR_EPSILON) {
          _vec6.copy(_edge.end);
          didIntersect = true;
        }
        if (didIntersect && !(_vec6.distanceTo(_edge.start) < EPSILON2)) {
          if (_vec6.distanceTo(_edge.end) < EPSILON2) {
            vertexSplitEnd = t;
          }
          if (intersects === 0) {
            _foundEdge.start.copy(_vec6);
          } else {
            _foundEdge.end.copy(_vec6);
          }
          intersects++;
        }
      }
...
var _normalA3 = new Vector3();
var _normalB3 = new Vector3();
function isTriangleCoplanar(triA, triB) {
  triA.getNormal(_normalA3);
  triB.getNormal(_normalB3);
  const dot = _normalA3.dot(_normalB3);
  if (Math.abs(1 - Math.abs(dot)) >= COPLANAR_NORMAL_EPSILON) {
    return false;
  }
  const dA = _normalA3.dot(triA.a);
  const dB = _normalA3.dot(triB.a);
...
function getCoplanarIntersectionEdges(triA, triB, target) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const result = clipSegmentToTriangle(_inputSeg, triA, _normalA3, _tempLine);
    if (result !== null) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const result = clipSegmentToTriangle(_inputSeg, triB, _normalB3, _tempLine);
    if (result !== null) {
      if (count >= target.length) {
        target.push(new Line3());
      }
...
function getHitSide(tri, bvh, matrix2 = null) {
  tri.getMidpoint(_ray7.origin);
  tri.getNormal(_ray7.direction);
  if (matrix2) {
    _ray7.origin.applyMatrix4(matrix2);
    _ray7.direction.transformDirection(matrix2);
  }
...
var _inverseMatrix6 = new Matrix4();
...
function performSplitTriangleOperations(a, b, intersectionMap, operations, invert, splitter, builders, groupOffset = 0) {
  ...
  _inverseMatrix6.copy(_matrix4).invert();
  if (invert) {
    _builderMatrix.copy(_matrix4);
  } else {
  ...
  for (let i = 0, l = splitIds.length; i < l; i++) {
    ...
    if (invert) {
      _triA.a.applyMatrix4(_matrix4);
      _triA.b.applyMatrix4(_matrix4);
      _triA.c.applyMatrix4(_matrix4);
    }
    splitter.reset();
    ...
    if (coplanarIndices) {
      for (const index of coplanarIndices) {
        ...
        if (!invert) {
          inst.a.applyMatrix4(_inverseMatrix6);
          inst.b.applyMatrix4(_inverseMatrix6);
          inst.c.applyMatrix4(_inverseMatrix6);
        }
    ...
    if (splitter.addConstraintEdge) {
      ...
      for (let ib = 0, l2 = intersectingIndices.length; ib < l2; ib++) {
        ...
        if (!invert) {
          _triB.a.applyMatrix4(_inverseMatrix6);
          _triB.b.applyMatrix4(_inverseMatrix6);
          _triB.c.applyMatrix4(_inverseMatrix6);
        }
    ...
    for (let ib = 0, l2 = triangles.length; ib < l2; ib++) {
      ...
      const raycastMatrix = invert ? null : _matrix4;
      let hitSide = null;
...
function performWholeTriangleOperations(a, b, splitTriSet, operations, invert, builders, groupOffset = 0) {
  ...
  for (let id = 0; id < triCount; id++) {
    ...
    if (invert) {
      _tri.a.applyMatrix4(_matrix4);
      _tri.b.applyMatrix4(_matrix4);
      _tri.c.applyMatrix4(_matrix4);
    }
    const hitSide = getHitSide(_tri, bBVH, invert ? null : _matrix4);
    _actions.length = 0;
    _builders.length = 0;
    for (let o = 0, lo = operations.length; o < lo; o++) {
      const op = getOperationAction(operations[o], hitSide, invert);
...
function pushItemSize(vec2, itemSize, target) {
  switch (itemSize) {
    case 1:
      target.push(vec2.x);
      break;
    case 2:
      target.push(vec2.x, vec2.y);
      break;
    case 3:
      target.push(vec2.x, vec2.y, vec2.z);
      break;
    case 4:
      target.push(vec2.x, vec2.y, vec2.z, vec2.w);
      break;
  }
}
...
var GeometryBuilder = class {
  ...
  initInterpolatedAttributeData(geometry, matrix2, normalMatrix, i0, i1, i2) {
    ...
    for (const key in attributeData) {
      ...
      let v0, v1, v2;
      if (key === "position") {
        v0 = _vec3_0.fromBufferAttribute(attr, i0).applyMatrix4(matrix2);
        v1 = _vec3_1.fromBufferAttribute(attr, i1).applyMatrix4(matrix2);
        v2 = _vec3_2.fromBufferAttribute(attr, i2).applyMatrix4(matrix2);
      } else if (key === "normal") {
        ...
        v0 = _vec3_0.fromBufferAttribute(attr, i0).transformDirection(matrix2);
        v1 = _vec3_1.fromBufferAttribute(attr, i1).transformDirection(matrix2);
        v2 = _vec3_2.fromBufferAttribute(attr, i2).transformDirection(matrix2);
      } else {
  ...
  appendIndexFromGeometry(geometry, matrix2, normalMatrix, group, index, invert = false) {
    ...
    if (index !== null && indexMap.has(index)) {
      ...
      for (const key in attributeData) {
        ...
        if (key === "position") {
          _vec32.fromBufferAttribute(attr, index).applyMatrix4(matrix2);
          ...
          _vec32.fromBufferAttribute(attr, index).transformDirection(matrix2);
          if (invert) {
            _vec32.multiplyScalar(-1);
          }
          arr.push(_vec32.x, _vec32.y, _vec32.z);
...
function getOverlapEdge(tri1, e1, tri2, e2, target) {
  ...
  let d0_1 = _vec7.subVectors(v0_1, _ray8.origin).dot(_ray8.direction);
  let d1_1 = _vec7.subVectors(v1_1, _ray8.origin).dot(_ray8.direction);
  if (d0_1 > d1_1) [d0_1, d1_1] = [d1_1, d0_1];
  let d0_2 = _vec7.subVectors(v0_2, _ray8.origin).dot(_ray8.direction);
  let d1_2 = _vec7.subVectors(v1_2, _ray8.origin).dot(_ray8.direction);
  ...
  setHalfEdges(geometry, halfEdges) {
    ...
    function addConnectionEdge(tri1, tri2, centerPoint) {
      ...
      if (straightEdges) {
        ...
        _vec7.addVectors(_center, _projectedDir);
        _vec23.addVectors(_center2, _projectedDir2);
        tri1.closestPointToPoint(_vec7, _finalPoint);
